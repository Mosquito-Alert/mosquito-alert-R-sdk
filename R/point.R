...
Point <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return Point as a base R list.
    #' @examples
    #' # convert array of Point (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert Point to a base R type
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`latitude`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Point: the required field `latitude` is missing."))
      }
      ...
      if (!is.null(input_json$`longitude`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Point: the required field `longitude` is missing."))
      }
    },
