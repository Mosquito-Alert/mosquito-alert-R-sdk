#' Create a new BitesGeoListReceivedAtErrorComponent
#'
#' @description
#' BitesGeoListReceivedAtErrorComponent Class
#'
#' @docType class
#' @title BitesGeoListReceivedAtErrorComponent
#' @description BitesGeoListReceivedAtErrorComponent Class
...
#' @export
BitesGeoListReceivedAtErrorComponent <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new BitesGeoListReceivedAtErrorComponent class.
    #'
    #' @param attr attr
    #' @param code code
    #' @param detail detail
    ...
    #'
    #' @return BitesGeoListReceivedAtErrorComponent as a base R list.
    #' @examples
    #' # convert array of BitesGeoListReceivedAtErrorComponent (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert BitesGeoListReceivedAtErrorComponent to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      BitesGeoListReceivedAtErrorComponentObject <- list()
      if (!is.null(self$`attr`)) {
        BitesGeoListReceivedAtErrorComponentObject[["attr"]] <-
          self$`attr`
      }
      if (!is.null(self$`code`)) {
        BitesGeoListReceivedAtErrorComponentObject[["code"]] <-
          self$`code`
      }
      if (!is.null(self$`detail`)) {
        BitesGeoListReceivedAtErrorComponentObject[["detail"]] <-
          self$`detail`
      }
      return(BitesGeoListReceivedAtErrorComponentObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of BitesGeoListReceivedAtErrorComponent
    #'
    #' @param input_json the JSON input
    #' @return the instance of BitesGeoListReceivedAtErrorComponent
    ...
    #' @return BitesGeoListReceivedAtErrorComponent in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to BitesGeoListReceivedAtErrorComponent and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for BitesGeoListReceivedAtErrorComponent: the required field `attr` is missing."))
      }
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for BitesGeoListReceivedAtErrorComponent: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for BitesGeoListReceivedAtErrorComponent: the required field `detail` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of BitesGeoListReceivedAtErrorComponent
    toString = function() {
      self$toJSONString()
    },
...
# BitesGeoListReceivedAtErrorComponent$unlock()
#
## Below is an example to define the print function
# BitesGeoListReceivedAtErrorComponent$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# BitesGeoListReceivedAtErrorComponent$lock()
