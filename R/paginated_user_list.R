#' Create a new PaginatedUserList
#'
#' @description
#' PaginatedUserList Class
#'
#' @docType class
#' @title PaginatedUserList
#' @description PaginatedUserList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{User})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedUserList <- R6::R6Class(
  "PaginatedUserList",
  public = list(
    `count` = NULL,
    `next` = NULL,
    ...
    #' @description
    #' Initialize a new PaginatedUserList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedUserList as a base R list.
    #' @examples
    #' # convert array of PaginatedUserList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedUserList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedUserListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedUserListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedUserListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedUserListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedUserListObject[["results"]] <-
          self$extractSimpleType(self$`results`)
      }
      return(PaginatedUserListObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of PaginatedUserList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedUserList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[User]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedUserList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedUserList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedUserList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedUserList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedUserList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedUserList$unlock()
#
## Below is an example to define the print function
# PaginatedUserList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedUserList$lock()
