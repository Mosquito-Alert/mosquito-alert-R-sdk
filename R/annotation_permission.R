#' Create a new AnnotationPermission
#'
#' @description
#' AnnotationPermission Class
#'
#' @docType class
#' @title AnnotationPermission
#' @description AnnotationPermission Class
...
#' @export
AnnotationPermission <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return AnnotationPermission as a base R list.
    #' @examples
    #' # convert array of AnnotationPermission (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert AnnotationPermission to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      AnnotationPermissionObject <- list()
      if (!is.null(self$`add`)) {
        AnnotationPermissionObject[["add"]] <-
          self$`add`
      }
      if (!is.null(self$`change`)) {
        AnnotationPermissionObject[["change"]] <-
          self$`change`
      }
      if (!is.null(self$`view`)) {
        AnnotationPermissionObject[["view"]] <-
          self$`view`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of AnnotationPermission
    #'
    #' @param input_json the JSON input
    #' @return the instance of AnnotationPermission
    ...
    #' @return AnnotationPermission in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to AnnotationPermission and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`add`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for AnnotationPermission: the required field `add` is missing."))
      }
      ...
      if (!is.null(input_json$`change`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for AnnotationPermission: the required field `change` is missing."))
      }
      ...
      if (!is.null(input_json$`view`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for AnnotationPermission: the required field `view` is missing."))
      }
    ...
    #'
    #' @return String representation of AnnotationPermission
    toString = function() {
      self$toJSONString()
    },
...
# AnnotationPermission$unlock()
#
## Below is an example to define the print function
# AnnotationPermission$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AnnotationPermission$lock()
