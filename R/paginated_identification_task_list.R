#' Create a new PaginatedIdentificationTaskList
#'
#' @description
#' PaginatedIdentificationTaskList Class
#'
#' @docType class
#' @title PaginatedIdentificationTaskList
#' @description PaginatedIdentificationTaskList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{IdentificationTask})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedIdentificationTaskList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedIdentificationTaskList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedIdentificationTaskList as a base R list.
    #' @examples
    #' # convert array of PaginatedIdentificationTaskList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedIdentificationTaskList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedIdentificationTaskListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedIdentificationTaskListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedIdentificationTaskListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedIdentificationTaskListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedIdentificationTaskListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedIdentificationTaskListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedIdentificationTaskList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedIdentificationTaskList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[IdentificationTask]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedIdentificationTaskList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedIdentificationTaskList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedIdentificationTaskList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedIdentificationTaskList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedIdentificationTaskList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedIdentificationTaskList$unlock()
#
## Below is an example to define the print function
# PaginatedIdentificationTaskList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedIdentificationTaskList$lock()
