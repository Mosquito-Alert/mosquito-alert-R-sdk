...
#' @docType class
#' @title PointRequest
#' @description PointRequest Class
...
PointRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PointRequest class.
    #'
    ...
    #'
    #' @return PointRequest as a base R list.
    #' @examples
    #' # convert array of PointRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PointRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PointRequestObject <- list()
      if (!is.null(self$`latitude`)) {
        PointRequestObject[["latitude"]] <-
          self$`latitude`
      }
      if (!is.null(self$`longitude`)) {
        PointRequestObject[["longitude"]] <-
          self$`longitude`
      }
      return(PointRequestObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PointRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of PointRequest
    ...
    #' @return PointRequest in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PointRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`latitude`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PointRequest: the required field `latitude` is missing."))
      }
      ...
      if (!is.null(input_json$`longitude`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PointRequest: the required field `longitude` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PointRequest
    toString = function() {
      self$toJSONString()
    },
...
# PointRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
