#' Create a new PaginatedWorkspaceList
#'
#' @description
#' PaginatedWorkspaceList Class
#'
#' @docType class
#' @title PaginatedWorkspaceList
#' @description PaginatedWorkspaceList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Workspace})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedWorkspaceList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedWorkspaceList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedWorkspaceList as a base R list.
    #' @examples
    #' # convert array of PaginatedWorkspaceList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedWorkspaceList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedWorkspaceListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedWorkspaceListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedWorkspaceListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedWorkspaceListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedWorkspaceListObject[["results"]] <-
          self$extractSimpleType(self$`results`)
      }
      return(PaginatedWorkspaceListObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of PaginatedWorkspaceList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedWorkspaceList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Workspace]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedWorkspaceList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedWorkspaceList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedWorkspaceList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedWorkspaceList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedWorkspaceList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedWorkspaceList$unlock()
#
## Below is an example to define the print function
# PaginatedWorkspaceList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedWorkspaceList$lock()
