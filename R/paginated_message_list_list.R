#' Create a new PaginatedMessageListList
#'
#' @description
#' PaginatedMessageListList Class
#'
#' @docType class
#' @title PaginatedMessageListList
#' @description PaginatedMessageListList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{MessageList})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedMessageListList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedMessageListList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedMessageListList as a base R list.
    #' @examples
    #' # convert array of PaginatedMessageListList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedMessageListList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedMessageListListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedMessageListListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedMessageListListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedMessageListListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedMessageListListObject[["results"]] <-
          self$extractSimpleType(self$`results`)
      }
      return(PaginatedMessageListListObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of PaginatedMessageListList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedMessageListList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[MessageList]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedMessageListList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedMessageListList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedMessageListList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedMessageListList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedMessageListList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedMessageListList$unlock()
#
## Below is an example to define the print function
# PaginatedMessageListList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedMessageListList$lock()
