...
Error415 <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new Error415 class.
    #'
    #' @param code code
    #' @param detail detail
    #' @param attr attr
    ...
    initialize = function(`code`, `detail`, `attr`, ...) {
      if (!missing(`code`)) {
        if (!(`code` %in% c("unsupported_media_type"))) {
          stop(paste("Error! \"", `code`, "\" cannot be assigned to `code`. Must be \"unsupported_media_type\".", sep = ""))
        }
    ...
    #' To JSON String
    #'
    #' @return Error415 in JSON format
    toJSON = function() {
      Error415Object <- list()
    ...
    #' @description
    #' Deserialize JSON string into an instance of Error415
    #'
    #' @param input_json the JSON input
    #' @return the instance of Error415
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`code`)) {
        if (!is.null(this_object$`code`) && !(this_object$`code` %in% c("unsupported_media_type"))) {
          stop(paste("Error! \"", this_object$`code`, "\" cannot be assigned to `code`. Must be \"unsupported_media_type\".", sep = ""))
        }
    ...
    #' @description
    #' Validate JSON input with respect to Error415 and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error415: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error415: the required field `detail` is missing."))
      }
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error415: the required field `attr` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of Error415
    toString = function() {
      self$toJSONString()
    },
...
# Error415$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
