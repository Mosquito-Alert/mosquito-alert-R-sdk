#' Create a new PaginatedTaxonList
#'
#' @description
#' PaginatedTaxonList Class
#'
#' @docType class
#' @title PaginatedTaxonList
#' @description PaginatedTaxonList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Taxon})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedTaxonList <- R6::R6Class(
  "PaginatedTaxonList",
  public = list(
    `count` = NULL,
    `next` = NULL,
    ...
    #' @description
    #' Initialize a new PaginatedTaxonList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedTaxonList as a base R list.
    #' @examples
    #' # convert array of PaginatedTaxonList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedTaxonList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedTaxonListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedTaxonListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedTaxonListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedTaxonListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedTaxonListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedTaxonListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedTaxonList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedTaxonList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Taxon]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedTaxonList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedTaxonList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedTaxonList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedTaxonList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedTaxonList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedTaxonList$unlock()
#
## Below is an example to define the print function
# PaginatedTaxonList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedTaxonList$lock()
