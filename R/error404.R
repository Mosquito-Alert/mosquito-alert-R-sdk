...
Error404 <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new Error404 class.
    #'
    #' @param code code
    #' @param detail detail
    #' @param attr attr
    ...
    initialize = function(`code`, `detail`, `attr`, ...) {
      if (!missing(`code`)) {
        if (!(`code` %in% c("not_found"))) {
          stop(paste("Error! \"", `code`, "\" cannot be assigned to `code`. Must be \"not_found\".", sep = ""))
        }
    ...
    #'
    #' @return Error404 as a base R list.
    #' @examples
    #' # convert array of Error404 (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert Error404 to a base R type
    #'
    ...
    #' @description
    #' Deserialize JSON string into an instance of Error404
    #'
    #' @param input_json the JSON input
    #' @return the instance of Error404
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`code`)) {
        if (!is.null(this_object$`code`) && !(this_object$`code` %in% c("not_found"))) {
          stop(paste("Error! \"", this_object$`code`, "\" cannot be assigned to `code`. Must be \"not_found\".", sep = ""))
        }
    ...
    #' @return Error404 in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to Error404 and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error404: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error404: the required field `detail` is missing."))
      }
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error404: the required field `attr` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of Error404
    toString = function() {
      self$toJSONString()
    },
...
# Error404$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
