...
#' @docType class
#' @title LocationPoint
#' @description LocationPoint Class
...
LocationPoint <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new LocationPoint class.
    #'
    ...
    #'
    #' @return LocationPoint as a base R list.
    #' @examples
    #' # convert array of LocationPoint (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert LocationPoint to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      LocationPointObject <- list()
      if (!is.null(self$`latitude`)) {
        LocationPointObject[["latitude"]] <-
          self$`latitude`
      }
      if (!is.null(self$`longitude`)) {
        LocationPointObject[["longitude"]] <-
          self$`longitude`
      }
      return(LocationPointObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of LocationPoint
    #'
    #' @param input_json the JSON input
    #' @return the instance of LocationPoint
    ...
    #' @return LocationPoint in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to LocationPoint and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`latitude`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocationPoint: the required field `latitude` is missing."))
      }
      ...
      if (!is.null(input_json$`longitude`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for LocationPoint: the required field `longitude` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of LocationPoint
    toString = function() {
      self$toJSONString()
    },
...
# LocationPoint$unlock()
#
## Below is an example to define the print function
# LocationPoint$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
