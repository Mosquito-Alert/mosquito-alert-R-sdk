...
#' @description AudienceFilter Class
...
AudienceFilter <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return AudienceFilter as a base R list.
    #' @examples
    #' # convert array of AudienceFilter (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert AudienceFilter to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      AudienceFilterObject <- list()
      if (!is.null(self$`last_login_before`)) {
        AudienceFilterObject[["last_login_before"]] <-
          self$`last_login_before`
      }
      if (!is.null(self$`last_login_after`)) {
        AudienceFilterObject[["last_login_after"]] <-
          self$`last_login_after`
      }
      if (!is.null(self$`in_area`)) {
        AudienceFilterObject[["in_area"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of AudienceFilter
    #'
    #' @param input_json the JSON input
    #' @return the instance of AudienceFilter
    ...
    #' @return AudienceFilter in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to AudienceFilter and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of AudienceFilter
    toString = function() {
      self$toJSONString()
    },
...
# AudienceFilter$unlock()
#
## Below is an example to define the print function
# AudienceFilter$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
