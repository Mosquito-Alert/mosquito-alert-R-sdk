#' Create a new TaxaListRankErrorComponent
#'
#' @description
#' TaxaListRankErrorComponent Class
#'
#' @docType class
#' @title TaxaListRankErrorComponent
#' @description TaxaListRankErrorComponent Class
...
#' @export
TaxaListRankErrorComponent <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new TaxaListRankErrorComponent class.
    #'
    #' @param attr attr
    #' @param code code
    #' @param detail detail
    ...
    initialize = function(`attr`, `code`, `detail`, ...) {
      if (!missing(`attr`)) {
        if (!(`attr` %in% c("rank"))) {
          stop(paste("Error! \"", `attr`, "\" cannot be assigned to `attr`. Must be \"rank\".", sep = ""))
        }
    ...
    #'
    #' @return TaxaListRankErrorComponent as a base R list.
    #' @examples
    #' # convert array of TaxaListRankErrorComponent (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert TaxaListRankErrorComponent to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      TaxaListRankErrorComponentObject <- list()
      if (!is.null(self$`attr`)) {
        TaxaListRankErrorComponentObject[["attr"]] <-
          self$`attr`
      }
      if (!is.null(self$`code`)) {
        TaxaListRankErrorComponentObject[["code"]] <-
          self$`code`
      }
      if (!is.null(self$`detail`)) {
        TaxaListRankErrorComponentObject[["detail"]] <-
          self$`detail`
      }
      return(TaxaListRankErrorComponentObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of TaxaListRankErrorComponent
    #'
    #' @param input_json the JSON input
    #' @return the instance of TaxaListRankErrorComponent
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`attr`)) {
        if (!is.null(this_object$`attr`) && !(this_object$`attr` %in% c("rank"))) {
          stop(paste("Error! \"", this_object$`attr`, "\" cannot be assigned to `attr`. Must be \"rank\".", sep = ""))
        }
    ...
    #' @return TaxaListRankErrorComponent in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to TaxaListRankErrorComponent and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for TaxaListRankErrorComponent: the required field `attr` is missing."))
      }
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for TaxaListRankErrorComponent: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for TaxaListRankErrorComponent: the required field `detail` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of TaxaListRankErrorComponent
    toString = function() {
      self$toJSONString()
    },
...
# TaxaListRankErrorComponent$unlock()
#
## Below is an example to define the print function
# TaxaListRankErrorComponent$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# TaxaListRankErrorComponent$lock()
