#' Create a new IdentificationTaskPermission
#'
#' @description
#' IdentificationTaskPermission Class
#'
#' @docType class
#' @title IdentificationTaskPermission
#' @description IdentificationTaskPermission Class
...
#' @export
IdentificationTaskPermission <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new IdentificationTaskPermission class.
    #'
    #' @param add add
    #' @param change change
    #' @param view view
    ...
    #'
    #' @return IdentificationTaskPermission as a base R list.
    #' @examples
    #' # convert array of IdentificationTaskPermission (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert IdentificationTaskPermission to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      IdentificationTaskPermissionObject <- list()
      if (!is.null(self$`add`)) {
        IdentificationTaskPermissionObject[["add"]] <-
          self$`add`
      }
      if (!is.null(self$`change`)) {
        IdentificationTaskPermissionObject[["change"]] <-
          self$`change`
      }
      if (!is.null(self$`view`)) {
        IdentificationTaskPermissionObject[["view"]] <-
          self$`view`
      }
      if (!is.null(self$`delete`)) {
        IdentificationTaskPermissionObject[["delete"]] <-
          self$`delete`
      }
      return(IdentificationTaskPermissionObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of IdentificationTaskPermission
    #'
    #' @param input_json the JSON input
    #' @return the instance of IdentificationTaskPermission
    ...
    #' @return IdentificationTaskPermission in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to IdentificationTaskPermission and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`add`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for IdentificationTaskPermission: the required field `add` is missing."))
      }
      ...
      if (!is.null(input_json$`change`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for IdentificationTaskPermission: the required field `change` is missing."))
      }
      ...
      if (!is.null(input_json$`view`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for IdentificationTaskPermission: the required field `view` is missing."))
      }
      ...
      if (!is.null(input_json$`delete`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for IdentificationTaskPermission: the required field `delete` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of IdentificationTaskPermission
    toString = function() {
      self$toJSONString()
    },
...
# IdentificationTaskPermission$unlock()
#
## Below is an example to define the print function
# IdentificationTaskPermission$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# IdentificationTaskPermission$lock()
