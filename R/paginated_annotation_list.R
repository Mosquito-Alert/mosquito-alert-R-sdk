#' Create a new PaginatedAnnotationList
#'
#' @description
#' PaginatedAnnotationList Class
#'
#' @docType class
#' @title PaginatedAnnotationList
#' @description PaginatedAnnotationList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Annotation})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedAnnotationList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedAnnotationList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedAnnotationList as a base R list.
    #' @examples
    #' # convert array of PaginatedAnnotationList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedAnnotationList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedAnnotationListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedAnnotationListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedAnnotationListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedAnnotationListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedAnnotationListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedAnnotationListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedAnnotationList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedAnnotationList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Annotation]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedAnnotationList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedAnnotationList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedAnnotationList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedAnnotationList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedAnnotationList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedAnnotationList$unlock()
#
## Below is an example to define the print function
# PaginatedAnnotationList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedAnnotationList$lock()
