#' Create a new ObservationsListReceivedAtErrorComponent
#'
#' @description
#' ObservationsListReceivedAtErrorComponent Class
#'
#' @docType class
#' @title ObservationsListReceivedAtErrorComponent
#' @description ObservationsListReceivedAtErrorComponent Class
...
#' @export
ObservationsListReceivedAtErrorComponent <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ObservationsListReceivedAtErrorComponent class.
    #'
    #' @param attr attr
    #' @param code code
    #' @param detail detail
    ...
    #'
    #' @return ObservationsListReceivedAtErrorComponent as a base R list.
    #' @examples
    #' # convert array of ObservationsListReceivedAtErrorComponent (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ObservationsListReceivedAtErrorComponent to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ObservationsListReceivedAtErrorComponentObject <- list()
      if (!is.null(self$`attr`)) {
        ObservationsListReceivedAtErrorComponentObject[["attr"]] <-
          self$`attr`
      }
      if (!is.null(self$`code`)) {
        ObservationsListReceivedAtErrorComponentObject[["code"]] <-
          self$`code`
      }
      if (!is.null(self$`detail`)) {
        ObservationsListReceivedAtErrorComponentObject[["detail"]] <-
          self$`detail`
      }
      return(ObservationsListReceivedAtErrorComponentObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ObservationsListReceivedAtErrorComponent
    #'
    #' @param input_json the JSON input
    #' @return the instance of ObservationsListReceivedAtErrorComponent
    ...
    #' @return ObservationsListReceivedAtErrorComponent in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ObservationsListReceivedAtErrorComponent and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ObservationsListReceivedAtErrorComponent: the required field `attr` is missing."))
      }
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ObservationsListReceivedAtErrorComponent: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ObservationsListReceivedAtErrorComponent: the required field `detail` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ObservationsListReceivedAtErrorComponent
    toString = function() {
      self$toJSONString()
    },
...
# ObservationsListReceivedAtErrorComponent$unlock()
#
## Below is an example to define the print function
# ObservationsListReceivedAtErrorComponent$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ObservationsListReceivedAtErrorComponent$lock()
