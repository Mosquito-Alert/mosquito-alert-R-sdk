...
PaginatedBiteList <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return PaginatedBiteList as a base R list.
    #' @examples
    #' # convert array of PaginatedBiteList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedBiteList to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      return(PaginatedBiteListObject)
    },
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedBiteList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedBiteList: the required field `results` is missing."))
      }
    },
