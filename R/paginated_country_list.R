#' Create a new PaginatedCountryList
#'
#' @description
#' PaginatedCountryList Class
#'
#' @docType class
#' @title PaginatedCountryList
#' @description PaginatedCountryList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Country})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedCountryList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedCountryList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedCountryList as a base R list.
    #' @examples
    #' # convert array of PaginatedCountryList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedCountryList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedCountryListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedCountryListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedCountryListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedCountryListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedCountryListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedCountryListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedCountryList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedCountryList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Country]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedCountryList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedCountryList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedCountryList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedCountryList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedCountryList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedCountryList$unlock()
#
## Below is an example to define the print function
# PaginatedCountryList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedCountryList$lock()
