...
ParseError <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ParseError class.
    #'
    #' @param code code
    #' @param detail detail
    #' @param attr attr
    ...
    initialize = function(`code`, `detail`, `attr`, ...) {
      if (!missing(`code`)) {
        if (!(`code` %in% c("parse_error"))) {
          stop(paste("Error! \"", `code`, "\" cannot be assigned to `code`. Must be \"parse_error\".", sep = ""))
        }
    ...
    #' To JSON String
    #'
    #' @return ParseError in JSON format
    toJSON = function() {
      ParseErrorObject <- list()
      if (!is.null(self$`code`)) {
        ParseErrorObject[["code"]] <-
          self$`code`
      }
      if (!is.null(self$`detail`)) {
        ParseErrorObject[["detail"]] <-
          self$`detail`
      }
      if (!is.null(self$`attr`)) {
        ParseErrorObject[["attr"]] <-
          self$`attr`
      }
      ParseErrorObject
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of ParseError
    #'
    #' @param input_json the JSON input
    #' @return the instance of ParseError
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`code`)) {
        if (!is.null(this_object$`code`) && !(this_object$`code` %in% c("parse_error"))) {
          stop(paste("Error! \"", this_object$`code`, "\" cannot be assigned to `code`. Must be \"parse_error\".", sep = ""))
        }
    ...
    #' @description
    #' Validate JSON input with respect to ParseError and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ParseError: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ParseError: the required field `detail` is missing."))
      }
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ParseError: the required field `attr` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ParseError
    toString = function() {
      self$toJSONString()
    },
...
# ParseError$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
