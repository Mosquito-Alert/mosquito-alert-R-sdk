#' Create a new PaginatedObservationList
#'
#' @description
#' PaginatedObservationList Class
#'
#' @docType class
#' @title PaginatedObservationList
#' @description PaginatedObservationList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Observation})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedObservationList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedObservationList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedObservationList as a base R list.
    #' @examples
    #' # convert array of PaginatedObservationList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedObservationList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedObservationListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedObservationListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedObservationListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedObservationListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedObservationListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedObservationListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedObservationList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedObservationList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Observation]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedObservationList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedObservationList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedObservationList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedObservationList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedObservationList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedObservationList$unlock()
#
## Below is an example to define the print function
# PaginatedObservationList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedObservationList$lock()
