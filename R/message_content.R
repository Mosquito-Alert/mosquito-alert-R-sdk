...
#' @description MessageContent Class
...
MessageContent <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new MessageContent class.
    #'
    ...
    #'
    #' @return MessageContent as a base R list.
    #' @examples
    #' # convert array of MessageContent (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MessageContent to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      MessageContentObject <- list()
      if (!is.null(self$`title`)) {
        MessageContentObject[["title"]] <-
          self$extractSimpleType(self$`title`)
      }
      if (!is.null(self$`body`)) {
        MessageContentObject[["body"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of MessageContent
    #'
    #' @param input_json the JSON input
    #' @return the instance of MessageContent
    ...
    #' @return MessageContent in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to MessageContent and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`title`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MessageContent: the required field `title` is missing."))
      }
      # check the required field `body`
      if (!is.null(input_json$`body`)) {
        stopifnot(R6::is.R6(input_json$`body`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for MessageContent: the required field `body` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of MessageContent
    toString = function() {
      self$toJSONString()
    },
...
# MessageContent$unlock()
#
## Below is an example to define the print function
# MessageContent$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
