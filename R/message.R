  ...
  public = list(
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Message: the required field `id` is missing."))
      }
      # check the required field `sender_user`
      if (!is.null(input_json$`sender_user`)) {
        stopifnot(R6::is.R6(input_json$`sender_user`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for Message: the required field `sender_user` is missing."))
      }
      ...
      if (!is.null(input_json$`created_at`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Message: the required field `created_at` is missing."))
      }
    },
