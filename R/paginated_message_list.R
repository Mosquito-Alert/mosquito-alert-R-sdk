#' Create a new PaginatedMessageList
#'
#' @description
#' PaginatedMessageList Class
#'
#' @docType class
#' @title PaginatedMessageList
#' @description PaginatedMessageList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Message})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedMessageList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedMessageList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedMessageList as a base R list.
    #' @examples
    #' # convert array of PaginatedMessageList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedMessageList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedMessageListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedMessageListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedMessageListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedMessageListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedMessageListObject[["results"]] <-
          self$extractSimpleType(self$`results`)
      }
      return(PaginatedMessageListObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of PaginatedMessageList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedMessageList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Message]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedMessageList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedMessageList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedMessageList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedMessageList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedMessageList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedMessageList$unlock()
#
## Below is an example to define the print function
# PaginatedMessageList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedMessageList$lock()
