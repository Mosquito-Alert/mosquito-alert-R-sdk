#' Create a new PaginatedNotificationList
#'
#' @description
#' PaginatedNotificationList Class
#'
#' @docType class
#' @title PaginatedNotificationList
#' @description PaginatedNotificationList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Notification})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedNotificationList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedNotificationList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedNotificationList as a base R list.
    #' @examples
    #' # convert array of PaginatedNotificationList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedNotificationList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedNotificationListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedNotificationListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedNotificationListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedNotificationListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedNotificationListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedNotificationListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedNotificationList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedNotificationList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Notification]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedNotificationList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedNotificationList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedNotificationList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedNotificationList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedNotificationList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedNotificationList$unlock()
#
## Below is an example to define the print function
# PaginatedNotificationList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedNotificationList$lock()
