#' Create a new MessageContentRequest
#'
#' @description
...
#' @description MessageContentRequest Class
...
#' @export
MessageContentRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new MessageContentRequest class.
    #'
    ...
    #'
    #' @return MessageContentRequest as a base R list.
    #' @examples
    #' # convert array of MessageContentRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MessageContentRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      MessageContentRequestObject <- list()
      if (!is.null(self$`title`)) {
        MessageContentRequestObject[["title"]] <-
          self$extractSimpleType(self$`title`)
      }
      if (!is.null(self$`body`)) {
        MessageContentRequestObject[["body"]] <-
          self$extractSimpleType(self$`body`)
      }
      return(MessageContentRequestObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of MessageContentRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of MessageContentRequest
    ...
    #' @return MessageContentRequest in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to MessageContentRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`title`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MessageContentRequest: the required field `title` is missing."))
      }
      # check the required field `body`
      if (!is.null(input_json$`body`)) {
        stopifnot(R6::is.R6(input_json$`body`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for MessageContentRequest: the required field `body` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of MessageContentRequest
    toString = function() {
      self$toJSONString()
    },
...
# MessageContentRequest$unlock()
#
## Below is an example to define the print function
# MessageContentRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MessageContentRequest$lock()
