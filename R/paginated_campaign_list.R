#' Create a new PaginatedCampaignList
#'
#' @description
#' PaginatedCampaignList Class
#'
#' @docType class
#' @title PaginatedCampaignList
#' @description PaginatedCampaignList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Campaign})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedCampaignList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedCampaignList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedCampaignList as a base R list.
    #' @examples
    #' # convert array of PaginatedCampaignList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedCampaignList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedCampaignListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedCampaignListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedCampaignListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedCampaignListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedCampaignListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedCampaignListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedCampaignList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedCampaignList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Campaign]", loadNamespace("MosquitoAlert"))
      }
      self
    },
    ...
    #' @return PaginatedCampaignList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedCampaignList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedCampaignList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedCampaignList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedCampaignList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedCampaignList$unlock()
#
## Below is an example to define the print function
# PaginatedCampaignList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedCampaignList$lock()
