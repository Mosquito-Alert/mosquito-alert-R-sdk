...
TopicNotificationCreateRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param receiver_type receiver_type. Default to "topic".
    #' @param ... Other optional arguments.
    initialize = function(`message`, `topic_codes`, `receiver_type` = "topic", ...) {
      if (!missing(`message`)) {
      ...
      if (!is.null(`receiver_type`)) {
        if (!(`receiver_type` %in% c("topic"))) {
          stop(paste("Error! \"", `receiver_type`, "\" cannot be assigned to `receiver_type`. Must be \"topic\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`receiver_type`)) {
        if (!is.null(this_object$`receiver_type`) && !(this_object$`receiver_type` %in% c("topic"))) {
          stop(paste("Error! \"", this_object$`receiver_type`, "\" cannot be assigned to `receiver_type`. Must be \"topic\".", sep = ""))
        }
