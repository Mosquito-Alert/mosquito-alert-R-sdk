#' Create a new MessagePermission
#'
#' @description
#' MessagePermission Class
#'
#' @docType class
#' @title MessagePermission
#' @description MessagePermission Class
...
MessagePermission <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new MessagePermission class.
    #'
    #' @param add add
    #' @param change change
    #' @param view view
    ...
    #'
    #' @return MessagePermission as a base R list.
    #' @examples
    #' # convert array of MessagePermission (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MessagePermission to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      MessagePermissionObject <- list()
      if (!is.null(self$`add`)) {
        MessagePermissionObject[["add"]] <-
          self$`add`
      }
      if (!is.null(self$`change`)) {
        MessagePermissionObject[["change"]] <-
          self$`change`
      }
      if (!is.null(self$`view`)) {
        MessagePermissionObject[["view"]] <-
          self$`view`
      }
      if (!is.null(self$`delete`)) {
        MessagePermissionObject[["delete"]] <-
          self$`delete`
      }
      return(MessagePermissionObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of MessagePermission
    #'
    #' @param input_json the JSON input
    #' @return the instance of MessagePermission
    ...
    #' @return MessagePermission in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to MessagePermission and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`add`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MessagePermission: the required field `add` is missing."))
      }
      ...
      if (!is.null(input_json$`change`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MessagePermission: the required field `change` is missing."))
      }
      ...
      if (!is.null(input_json$`view`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MessagePermission: the required field `view` is missing."))
      }
      ...
      if (!is.null(input_json$`delete`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MessagePermission: the required field `delete` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of MessagePermission
    toString = function() {
      self$toJSONString()
    },
...
# MessagePermission$unlock()
#
## Below is an example to define the print function
# MessagePermission$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MessagePermission$lock()
