...
Error405 <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new Error405 class.
    #'
    #' @param code code
    #' @param detail detail
    #' @param attr attr
    ...
    initialize = function(`code`, `detail`, `attr`, ...) {
      if (!missing(`code`)) {
        if (!(`code` %in% c("method_not_allowed"))) {
          stop(paste("Error! \"", `code`, "\" cannot be assigned to `code`. Must be \"method_not_allowed\".", sep = ""))
        }
    ...
    #' To JSON String
    #'
    #' @return Error405 in JSON format
    toJSON = function() {
      Error405Object <- list()
    ...
    #' @description
    #' Deserialize JSON string into an instance of Error405
    #'
    #' @param input_json the JSON input
    #' @return the instance of Error405
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`code`)) {
        if (!is.null(this_object$`code`) && !(this_object$`code` %in% c("method_not_allowed"))) {
          stop(paste("Error! \"", this_object$`code`, "\" cannot be assigned to `code`. Must be \"method_not_allowed\".", sep = ""))
        }
    ...
    #' @description
    #' Validate JSON input with respect to Error405 and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`code`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error405: the required field `code` is missing."))
      }
      ...
      if (!is.null(input_json$`detail`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error405: the required field `detail` is missing."))
      }
      ...
      if (!is.null(input_json$`attr`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for Error405: the required field `attr` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of Error405
    toString = function() {
      self$toJSONString()
    },
...
# Error405$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
